...
def load_all_jsons(input_dir="data/2_silver", output_dir="data/3_gold"):
    # Create gold folder if it does not exist, if it already exists, do nothing
    os.makedirs(output_dir, exist_ok=True)
    # Store under data/3_gold/jobs.db
    db_path = os.path.join(output_dir, "jobs.db")

    # Connect to SQLite database
    connection = sqlite3.connect(db_path) # Link Python to SQLite
    cursor = connection.cursor() # Run SQL commands
